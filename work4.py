...
@ti.func
def compute_phong(P, N, V, obj_color, Ka, Kd, Ks, shininess):
    """ Phong 着色器计算 """
    # 归一化光线方向 L
    L = tm.normalize(light_pos - P)
    # 计算理想反射向量 R = 2(N·L)N - L
    R = tm.normalize(2.0 * tm.dot(N, L) * N - L)
    
    # 1. 纯环境光 (Ambient)
    ambient = Ka * light_color * obj_color
    
    # 2. 漫反射 (Diffuse) - Lambert定律
    ...
    if tm.dot(N, L) > 0.0:  # 只有在迎光面才计算高光
        spec_factor = ti.pow(ti.max(0.0, tm.dot(R, V)), shininess)
    ...
    for i, j in pixels:
        ...
        if hit_obj > 0:
            ...
            if hit_obj == 1:
                ...
                pixel_color = compute_phong(P, N, V, sphere_color, Ka, Kd, Ks, shininess)
            elif hit_obj == 2:
                # 圆锥法线直接使用求交时返回的结果
                N = N_cone
                pixel_color = compute_phong(P, N, V, cone_color, Ka, Kd, Ks, shininess)
